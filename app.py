...
def _norm_news(title):
    ...
    # توحيد أشكال الألف/الياء والتشكيل حتى لا تفلت العناوين من الفلتر
    t = re.sub(r'[إأآ]', 'ا', t)
...
def is_earnings_news(title):
    ...
    if len(t) < 25:
        return False
    reject = ['اجازه', 'عطله', 'اليوم الوطني', 'تحليل فني', 'توقعات المحللين', 'الحصه السوقيه', 'متوسط سعر البيع', 'مكررات الارباح', 'اخر 12 شهر', 'توزيعات الارباح النقديه 2025', 'مكررات', 'بيانات تاريخيه', 'قائمه التوزيعات']
...
def is_material_saudi_news(title):
    ...
    if len(t) < 25:
        return False
    reject = ['اجازه', 'عطله', 'اليوم الوطني', 'التحليل الفني', 'تحليل فني', 'رصد للاسهم', 'ادني سعر منذ الادراج', 'اعلي سعر منذ الادراج', 'بيوت الخبره', 'توقعات المحللين', 'متوسط سعر البيع', 'الحصه السوقيه', 'مكررات الارباح', 'اخر 12 شهر', 'توزيعات الارباح النقديه 2025', 'مكررات', 'بيانات تاريخيه', 'قائمه التوزيعات']
    if any(k in t for k in reject):
        return False
...
def send_news_now():
    ...
    try:
        ...
        def unique(items):
            out = []
            for x in items:
                # منع نفس الحدث بين الأقسام حتى لو اختلفت الصياغة أو المصدر
                if any(is_similar_news(x, old) for old in seen_titles):
                    continue
                seen_titles.append(x)
                out.append(x)
            return out
